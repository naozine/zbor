...
## Appendix F: タイムライン型トランスクリプト表示

### 背景

現在のセグメント表示では、音声のセグメントがそのまま表示されるため：
- セグメント間に時間の飛びがある（例: 9.82秒 → 13.65秒）
- 無音区間がどこにあるか視覚的に分からない
- テキストの密度が一様に見えてしまう

### 要件

1. **連続タイムライン表示**
   - 音声全体を一定間隔（例: 10秒）で区切った「表示セグメント」として表示
   - 時間の飛びをなくし、連続的なタイムラインを提供
...
3. **テキスト位置の可視化**
   - 表示セグメント内でのテキストの相対位置が分かるように
   - 文字クリックで該当位置から再生（既存機能維持）

### 表示形式

**メイン表示（一定間隔タイムライン）:**
```
[00:00-00:10] 学年末はまだ二月に入試の休みがあるので・・・・
[00:10-00:20] ・・・・・休みも挟みますからいいんですけど・・今回が一番響かと思います
[00:20-00:30] ・・・・・・・・・・・・・・・・・・・・
[00:30-00:40] 普通十月の中頃に中間をやるんですけど・・多分
[00:40-00:50] ・二期生なので・・・形・・・・・はい
[00:50-01:00] ・・ありました・・・落とすの
```
...
[00:10-00:20] ・・・・・休みも挟みますからいいんですけど・・今回が一番響かと思います
              └─ seg2: 13.65-17.21 ─┘└─ seg3: 17.81-20.94 ─┘
```

- 元のASRセグメント境界を下線で表示
- チェックボックスで表示ON/OFF切替可能
...
type DisplaySegment struct {
    StartTime    float64   // 表示セグメントの開始時刻
    EndTime      float64   // 表示セグメントの終了時刻
    Elements     []Element // 表示要素（テキストまたは無音）
}

type Element struct {
    Type      string  // "text" or "silence"
    Text      string  // テキスト内容（type=text時）
    StartTime float64 // 開始時刻
    Duration  float64 // 長さ
}
```
...
│ [▶] ━━━━━━━━━●━━━━━━━━━━━━━━━━ 2:15 / 5:30  [1x ▼]    │
├─────────────────────────────────────────────────────────┤
│ Interval: [10s ▼]   ☑ Show segment info                 │
├─────────────────────────────────────────────────────────┤
│ [00:00-00:10] 学年末はまだ二月に入試の休みがあるので・・│
│               └── seg1: 0.13-9.82 ─────────────────┘    │
│ [00:10-00:20] ・・・・休みも挟みますから...            │ ← 現在再生中
│               └─ seg2: 13.65-17.21 ─┘                   │
│ [00:20-00:30] ・・・・・・・・・・・・・・             │
│ [00:30-00:40] 普通十月の中頃に...                       │
└─────────────────────────────────────────────────────────┘
```

**コントロール:**
- `Interval`: 表示間隔（5s/10s/30s/60s）
- `Show segment info`: ASRセグメント情報の表示ON/OFF

**レイアウト:**
- 再生コントロール部分は `position: sticky; top: 0` で固定
- スクロールしてもコントロールは常時表示
- トランスクリプト部分のみスクロール
...
| 操作 | 動作 |
|------|------|
| テキストクリック | その文字の時刻から再生開始 |
| `・` クリック | その無音区間の開始時刻から再生 |
| セグメントヘッダクリック | そのセグメント開始時刻から再生 |

### 色分け

| 要素 | 色 |
|------|-----|
| 現在再生中のセグメント | 薄青背景 `bg-blue-50` |
| 現在再生中の文字 | 黄色背景 `bg-yellow-200` |
| 無音マーカー `・` | 薄いグレー `text-gray-300` |
| 時刻ラベル | グレー `text-gray-400` |

### 実装ステップ

1. **DisplaySegment生成ロジック** (`internal/asr/display.go`)
   - トークンを時間順にソート
   - 表示セグメント幅で分割
   - トークン間ギャップに無音マーカー挿入

2. **テンプレート更新** (`web/components/transcript_sync.templ`)
   - 新しい表示形式に対応
   - 無音マーカーのクリックハンドリング
...
## Appendix G: VAD+ブロックベース文字起こし

### 背景・課題

現在のチャンクベース処理では、以下の問題が発生している：

1. **タイムスタンプの不正確さ**: 固定20秒チャンクで処理すると、無音区間を跨ぐ際にタイムスタンプがずれる
2. **無音区間での誤認識**: チャンク内に無音があると、モデルが不正確なタイムスタンプを推定
3. **チャンク境界問題**: 音声が途中で切れ、認識精度が低下

**具体例:**
```
音声: "...す" → 約20秒の無音 → "なん..."
期待: "す"=158s, "なん"=179s
実際: "す"=158s, "な"=169s(誤), "ん"=179s
```

### 解決策: VAD+ブロックベース処理

**基本アイデア:**
音声活動検出（VAD）で発話区間を検出し、各発話ブロックを単位として文字起こしする。

```
入力音声: [発話1]----無音----[発話2]----無音----[発話3]
             ↓                  ↓                  ↓
VAD検出:  Block1(0-10s)    Block2(30-45s)    Block3(60-80s)
             ↓                  ↓                  ↓
文字起こし:  Result1           Result2           Result3
             ↓                  ↓                  ↓
結合:     全結果をタイムスタンプ順にマージ
```
...
│                                                          │
│   ┌─────────────┐  ┌─────────────┐  ┌─────────────┐     │
│   │ Recognizer1 │  │ Recognizer2 │  │ Recognizer3 │     │
│   └─────────────┘  └─────────────┘  └─────────────┘     │
│         │                │                │              │
│         ▼                ▼                ▼              │
...
| 項目 | 改善前（チャンク方式） | 改善後（VAD方式） |
|------|----------------------|------------------|
| タイムスタンプ精度 | 無音区間でずれる | 高精度 |
| 境界問題 | 発生する | なし |
| 無音区間処理 | 不要な処理が走る | スキップ |
| 処理効率 | 一定 | 発話区間のみ処理 |

### 優先度

Phase 1: 即座に対応が必要（タイムスタンプ精度問題の根本解決）
Phase 2: 性能要件に応じて後日実装
